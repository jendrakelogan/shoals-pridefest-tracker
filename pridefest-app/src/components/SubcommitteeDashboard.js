...
function TaskCard({ task, catId, ti, totalTasks, isMobile, scColor, onUpdate, onDelete, onMoveUp, onMoveDown }) {
  ...
  const updatedLabel = timeAgo(task.updated_at)

  const ReorderButtons = () => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '2px', flexShrink: 0 }}>
      <button onClick={() => onMoveUp(catId, ti)} disabled={ti === 0}
        style={{ background: ti === 0 ? '#F3F4F6' : scColor, border: 'none', borderRadius: '4px', color: ti === 0 ? '#D1D5DB' : '#FFF', fontSize: '10px', padding: '2px 5px', cursor: ti === 0 ? 'default' : 'pointer', lineHeight: 1 }}>▲</button>
      <button onClick={() => onMoveDown(catId, ti)} disabled={ti === totalTasks - 1}
        style={{ background: ti === totalTasks - 1 ? '#F3F4F6' : scColor, border: 'none', borderRadius: '4px', color: ti === totalTasks - 1 ? '#D1D5DB' : '#FFF', fontSize: '10px', padding: '2px 5px', cursor: ti === totalTasks - 1 ? 'default' : 'pointer', lineHeight: 1 }}>▼</button>
    </div>
  )

  if (isMobile) {
    return (
      <div style={{ padding: '14px 16px', borderBottom: ti < totalTasks - 1 ? '1px solid #F3F4F6' : 'none', background: ti % 2 === 1 ? '#FAFAFA' : '#FFF' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '10px' }}>
          ...
          {updatedLabel && (
            <span style={{ fontSize: '11px', color: '#9CA3AF', marginLeft: 'auto', fontStyle: 'italic' }}>✎ {updatedLabel}</span>
          )}
          <button onClick={() => onDelete(catId, task.id)}
            style={{ background: 'none', border: 'none', color: '#D1D5DB', fontSize: '18px', padding: '0 4px' }}>✕</button>
        </div>
  ...
  // Desktop
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '44px 1fr 180px 130px 1fr 100px 40px', padding: '12px 20px', borderBottom: ti < totalTasks - 1 ? '1px solid #F3F4F6' : 'none', background: ti % 2 === 1 ? '#FAFAFA' : '#FFF', alignItems: 'center' }}>
      <ReorderButtons />
      <textarea value={task.description} onChange={e => onUpdate(catId, task.id, 'description', e.target.value)}
        placeholder="What needs to be done..." rows={2}
      ...
      <div style={{ fontSize: '11px', color: '#9CA3AF', fontStyle: 'italic', textAlign: 'center', paddingRight: '8px' }}>
...
export default function SubcommitteeDashboard({ subcommittee, onLogout }) {
  ...
  useEffect(() => { loadData() }, [sc.id]) // eslint-disable-line react-hooks/exhaustive-deps

  async function loadData() {
    setLoading(true)
    const { data: cats } = await supabase.from('categories').select('*').eq('subcommittee_id', sc.id).order('created_at')
    if (!cats) { setLoading(false); return }
    const { data: tasks } = await supabase.from('tasks').select('*').in('category_id', cats.map(c => c.id)).order('sort_order', { ascending: true })
    setCategories(cats.map(cat => ({ ...cat, tasks: (tasks || []).filter(t => t.category_id === cat.id) })))
    setLoading(false)
  }
  ...
  async function addCategory() {
    ...
    const { data, error } = await supabase.from('categories').insert({ subcommittee_id: sc.id, name: newCatName.trim() }).select().single()
  ...
  async function addTask(catId) {
    ...
    const { data, error } = await supabase.from('tasks').insert({
      category_id: catId, description: '', lead: '', status: 'Not Started', progress: '', sort_order: cat ? cat.tasks.length : 0
    }).select().single()
    if (!error && data) setCategories(prev => prev.map(c => c.id === catId ? { ...c, tasks: [...c.tasks, data] } : c))
  }

  async function updateTask(catId, taskId, field, value) {
    const now = new Date().toISOString()
    setCategories(prev => prev.map(c =>
      c.id === catId ? { ...c, tasks: c.tasks.map(t => t.id === taskId ? { ...t, [field]: value, updated_at: now } : t) } : c
    ))
    await supabase.from('tasks').update({ [field]: value, updated_at: now }).eq('id', taskId)
  }

  async function deleteTask(catId, taskId) {
    await supabase.from('tasks').delete().eq('id', taskId)
    setCategories(prev => prev.map(c => c.id === catId ? { ...c, tasks: c.tasks.filter(t => t.id !== taskId) } : c))
  }
      ...
      <div style={{ background: sc.color }}>
        <div style={{ maxWidth: '960px', margin: '0 auto', padding: '28px 24px 24px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '12px' }}>
            <div>
              <div style={{ fontSize: '12px', fontWeight: '600', letterSpacing: '0.15em', color: 'rgba(255,255,255,0.7)', textTransform: 'uppercase', marginBottom: '6px' }}>🌈 Shoals PrideFest 2026</div>
              <h1 style={{ fontSize: 'clamp(22px, 4vw, 34px)', color: '#FFF', lineHeight: 1.1 }}>{sc.emoji} {sc.label}</h1>
            </div>
            <button onClick={onLogout} style={{ padding: '9px 18px', background: 'rgba(0,0,0,0.2)', border: '1.5px solid rgba(255,255,255,0.3)', borderRadius: '10px', color: '#FFF', fontSize: '13px', fontWeight: '600' }}>← Switch Subcommittee</button>
          </div>
        </div>
      </div>
        ...
        {loading ? (
          ...
          <>
            ...
            {categories.map((cat, ci) => (
              <div key={cat.id} className="fade-up" style={{ animationDelay: `${ci * 0.05}s`, background: '#FFF', borderRadius: '18px', border: '1.5px solid #E5E7EB', marginBottom: '24px', overflow: 'hidden', boxShadow: '0 2px 12px rgba(0,0,0,0.05)' }}>
                ...
                {!isMobile && (
                  <div style={{ display: 'grid', gridTemplateColumns: '44px 1fr 180px 130px 1fr 100px 40px', background: '#F9FAFB', borderBottom: '1.5px solid #E5E7EB', padding: '10px 20px' }}>
                    {['', 'Task — What Needs to Be Done', 'Taking the Lead', 'Status', 'Progress Notes', 'Last Updated', ''].map((h, i) => (
                      <div key={i} style={{ fontSize: '11px', fontWeight: '700', color: '#9CA3AF', letterSpacing: '0.08em', textTransform: 'uppercase', paddingRight: i < 6 ? '12px' : '0' }}>{h}</div>
                    ))}
                  </div>
                )}
                ...
                {cat.tasks.map((task, ti) => (
                  <TaskCard key={task.id} task={task} catId={cat.id} ti={ti} totalTasks={cat.tasks.length}
                    isMobile={isMobile} scColor={sc.color} onUpdate={updateTask} onDelete={deleteTask}
                    onMoveUp={handleMoveUp} onMoveDown={handleMoveDown} />
                ))}

                <div style={{ padding: '12px 20px', borderTop: '1px solid #F3F4F6' }}>
                  <button onClick={() => addTask(cat.id)} style={{ background: 'none', border: `1.5px dashed ${sc.color}`, color: sc.color, borderRadius: '10px', padding: '8px 18px', fontSize: '13px', fontWeight: '600', width: '100%', transition: 'background 0.15s' }}
                    onMouseOver={e => e.target.style.background = `${sc.color}15`} onMouseOut={e => e.target.style.background = 'none'}>+ Add Task</button>
                </div>
              </div>
            ))}
            ...
            {addingCat ? (
              ...
              <button onClick={() => setAddingCat(true)} style={{ width: '100%', padding: '16px', background: sc.color, border: 'none', borderRadius: '14px', color: '#FFF', fontSize: '16px', fontWeight: '700', fontFamily: 'Playfair Display, serif', boxShadow: `0 4px 20px ${sc.color}55`, transition: 'opacity 0.2s, transform 0.2s' }}
                onMouseOver={e => { e.target.style.opacity='0.88'; e.target.style.transform='translateY(-1px)' }}
                onMouseOut={e => { e.target.style.opacity='1'; e.target.style.transform='translateY(0)' }}>+ Add New Category</button>
            )}
          </>
        )}
      </div>
