...
export default function SubcommitteeDashboard({ subcommittee, onLogout }) {
  ...
  function handleDragStart(catId, taskIndex) {
    dragTask.current = { catId, taskIndex }
  }

  function handleDragEnter(catId, taskIndex) {
    dragOver.current = { catId, taskIndex }
  }
  ...
  async function addTask(catId) {
    ...
    const sortOrder = cat ? cat.tasks.length : 0
  ...
  async function updateTask(catId, taskId, field, value) {
    setCategories(prev => prev.map(c =>
      c.id === catId
        ? { ...c, tasks: c.tasks.map(t => t.id === taskId ? { ...t, [field]: value } : t) }
        : c
    ))
          ...
          <>
            {categories.length === 0 && (
              <div style={{
                ...
                border: '2px dashed #D1D5DB', borderRadius: '18px', marginBottom: '24px'
              }}>
                ...
                <p style={{ color: '#9CA3AF', fontSize: '14px' }}>
                  Add your first category below — like "Art Exhibit" or "Drag Show Decor"
                </p>
              </div>
            )}
            ...
            {categories.map((cat, ci) => (
              <div key={cat.id} className="fade-up" style={{
                ...
                <div style={{
                  background: sc.color, padding: '16px 20px',
                  display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                }}>
                  <h2 style={{ fontSize: '18px', color: '#FFF', fontFamily: 'Playfair Display, serif' }}>
                    {cat.name}
                  </h2>
                ...
                <div style={{
                  ...
                  {['', 'Task — What Needs to Be Done', 'Taking the Lead', 'Status', 'Progress Notes', ''].map((h, i) => (
                    <div key={i} style={{
                      ...
                      paddingRight: i < 5 ? '12px' : '0'
                    }}>{h}</div>
                  ))}
                </div>

                {cat.tasks.length === 0 && (
                  <div style={{ padding: '20px', color: '#D1D5DB', fontSize: '14px', textAlign: 'center' }}>
                ...
                {cat.tasks.map((task, ti) => {
                  ...
                  return (
                    <div
                      key={task.id}
                      draggable
                      onDragStart={() => handleDragStart(cat.id, ti)}
                      onDragEnter={() => handleDragEnter(cat.id, ti)}
                      ...
                      style={{
                        ...
                        gridTemplateColumns: '28px 1fr 180px 130px 1fr 40px',
                        padding: '12px 20px',
                        borderBottom: ti < cat.tasks.length - 1 ? '1px solid #F3F4F6' : 'none',
                        background: ti % 2 === 1 ? '#FAFAFA' : '#FFF',
                        alignItems: 'start',
                      }}
                    >
                      <div style={{
                        color: '#C0C0C0', fontSize: '18px', paddingTop: '4px',
                      ...
                      <textarea
                        ...
                        style={{
                          ...
                          fontSize: '14px', color: '#1F2937', resize: 'vertical',
                          outline: 'none', paddingRight: '12px', lineHeight: '1.5', cursor: 'text',
                        }}
                      />
                      <input
                        value={task.lead}
                        onChange={e => updateTask(cat.id, task.id, 'lead', e.target.value)}
                        placeholder="Name..."
                        style={{
                          width: '100%', border: 'none', background: 'transparent',
                          fontSize: '14px', color: '#374151', outline: 'none',
                          paddingRight: '12px', cursor: 'text',
                        }}
                      />
                      <select
                        value={task.status}
                        onChange={e => updateTask(cat.id, task.id, 'status', e.target.value)}
                        style={{
                          border: 'none', borderRadius: '8px', padding: '4px 8px',
                          fontSize: '12px', fontWeight: '700', cursor: 'pointer',
                          background: ss.bg, color: ss.color, outline: 'none', marginRight: '12px',
                        }}
                      >
                      ...
                      <textarea
                        value={task.progress}
                        onChange={e => updateTask(cat.id, task.id, 'progress', e.target.value)}
                        placeholder="Progress notes, updates, blockers..."
                        rows={2}
                        style={{
                          width: '100%', border: 'none', background: 'transparent',
                          fontSize: '13px', color: '#6B7280', resize: 'vertical',
                          outline: 'none', fontStyle: 'italic', lineHeight: '1.5', cursor: 'text',
                        }}
                      />
                      <button
                        ...
                        style={{
                          ...
                          fontSize: '16px', padding: '4px', borderRadius: '6px', transition: 'color 0.15s',
                        }}
                ...
                <div style={{ padding: '12px 20px', borderTop: '1px solid #F3F4F6' }}>
                  <button
                    ...
                    style={{
                      background: 'none', border: `1.5px dashed ${sc.color}`,
                      color: sc.color, borderRadius: '10px', padding: '8px 18px',
                      fontSize: '13px', fontWeight: '600', width: '100%', transition: 'background 0.15s',
                    }}
            ...
            {addingCat ? (
              <div className="fade-in" style={{
                ...
                border: `2px solid ${sc.color}`, display: 'flex', gap: '12px', alignItems: 'center',
              }}>
                <input
                  autoFocus
                  value={newCatName}
                  ...
                  style={{ flex: 1, border: 'none', fontSize: '16px', outline: 'none', color: '#1F2937' }}
                />
                <button onClick={addCategory} style={{
                  background: sc.color, border: 'none', color: '#FFF',
                ...
                <button onClick={() => { setAddingCat(false); setNewCatName('') }} style={{
                  ...
                  color: '#9CA3AF', borderRadius: '10px', padding: '10px 16px', fontSize: '14px',
                }}>Cancel</button>
              </div>
            ) : (
              <button
